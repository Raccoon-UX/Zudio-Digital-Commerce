# ZOA Digital Commerce (Concept Pilot) — Live Demonstration Playbook

A structured, end-to-end presentation walkthrough for showcasing the **ZOA Digital Commerce — Concept Pilot** architecture to executive stakeholders, product teams, and engineering evaluators.
...
|---|---|---|---|
| **Executive Admin** | `admin@zoa.demo` | `Admin@12345` | Real-time KPI Dashboard, Store Inventory Matrix, Order Fulfillment progression, User Role Promotion, Audit Logs. |
| **Store Staff (POS)** | `staff.blr@zoa.demo` | `Staff@12345` | Scoped to **ZOA Indiranagar (Bengaluru)**. Pickup pass lookup, "Mark Ready", "Complete Handover" stock consumption. |
| **Online Customer** | `customer@zoa.demo` | `Customer@12345` | Catalog browsing, dual-mode cart, wishlist, checkout, live order tracking, 2-hour store reservation hold passes. |
...
3. **2-Hour In-Store Reservation Hold:**
   - Select size **L**, choose **ZOA Indiranagar (Bengaluru)**, and click **"Reserve for Store Pickup"**.
   - Immediate confirmation: Real-time pickup pass generated with pickup code (e.g. `ZOA-8F2Q`) and a live **2-hour countdown timer**.
...
1. **Store Staff Authentication:**
   - Sign in as `staff.blr@zoa.demo`.
   - Navigate to `/staff/reservations`.
2. **Pickup Code Verification:**
   - Search the customer's pickup code `ZOA-8F2Q`.
...
1. **Dual-Mode Cart & Merging:**
   ...
   - Click **Sign In** (`customer@zoa.demo`).
...
2. **Single-Store Fulfillment Allocation:**
   ...
   - Server automatically selects `ZOA Indiranagar (Bengaluru)` with 100% item availability.
...
1. **Executive KPI Dashboard:**
   - Sign in as `admin@zoa.demo` and navigate to `/admin`.
